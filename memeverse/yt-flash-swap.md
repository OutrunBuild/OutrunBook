...
Memeverse 不为 YT 设立独立的交易池，但 YT 始终可以交易：通过复用 PT/POL 池流动性的 **YT Flash Swap**，在锁定期内用 POL 买入 YT、或卖出 YT 换回 POL。整笔兑换在单笔原子交易内完成 —— 换腿所需的资金由池子临时垫付、交易结束前归还，对用户不可见，所以叫"闪电"。
...
> **1 个 YT 的价格（以 POL 计）= 1 个 POL − 1 个 PT 的市价**

PT/POL 池里 PT 有市价，YT 的价格就被唯一确定，不必再为 YT 单独建池。PT 越接近 POL 平价（市场越看好本金兑付），YT 越便宜；PT 折价越深，YT 越贵。本质上，YT 是对**结算残值**（结算回收扣除 PT 本金准备后的剩余，详见 [POL 拆分](pol-splitter.md)）的市场定价 —— 预期残值越高，YT 越贵。
...
- **滑点保护**：买入设最高支付上限，卖出设最低收到下限，另有过期时限，超出即整笔回滚。
- **单笔原子执行**：要么全部完成，要么分毫不差回滚 —— 不会出现「扣了钱没拿到 YT」或「YT 没了钱也没到」。
- **不预扣、不退款**：买入只拉取实际成本，上限只是护栏。
- **诚实失败**：受手续费、舍入和池子深度影响，某些目标数量可能没有可行成交价，交易会直接回滚，重试或调整数量即可，不会产生损失。
...
- 交易须由**智能账户**（如 Safe 等账户抽象钱包）在单次原子调用内发起，前端会代为处理会话包裹 —— 这也是 Memeverse 交易层的统一要求。
...
> 某 Memecoin 创世锁定后，PT 市价约 0.9 POL，YT 市价约 0.1 POL。一位用户用 1,000 POL 买入 10,000 YT（设好最高支付额），等于花一小部分价格拿到 10,000 份结算残值的完整暴露：结算残值每比预期多 0.01 POL，他就多赚 100 POL（相对本金约 10%）；残值比预期少则亏损同样放大，Memecoin 大跌、残值降低时 YT 价值也会降低。
>
