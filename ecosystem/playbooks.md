# 参与方式与风险偏好

Outrun 不是只有一个入口、一种玩法。不同风险偏好的人，在生态里有完全不同的参与路径。下面按**三档**讲清楚：每档适合谁、怎么参与、赚什么、承担什么风险，并给出带数字的例子。

## 全景：按风险选你的位置

| 档位 | 你想做什么 | 主要参与 | 收益来源 | 下行风险 |
|---|---|---|---|---|
| **稳健型** | 保本赚收益 | OutStake 质押铸 uAsset + 普通创世 | 底层生息增值 + YT + 辅助池手续费 | **无**（uAsset 本金守恒，与 Memecoin 涨跌无关） |
| **平衡型** | 放大早期敞口 | 杠杆创世 + 预购 | YT + 结算残值 + 预购份额 | 仅利息成本 |
| **共建型** | 深度参与社区 | 买入 Memecoin + 质押 + DAO 治理 | Staking 手续费 + 周期激励 + 治理权 + 上涨 | Memecoin 价格波动 |

> ⚠️ 关键卖点：**稳健型全程无风险**。普通创世投入的 uAsset 在四池里守恒 —— Memecoin 是全新铸造的，归零只损失"Memecoin 那部分预期收益"，投入的 uAsset 本金按份额拿回，一分不少。这是 Outrun 给稳定币用户的「增强收益」：本金安全，额外白拿 Memecoin 上行。
>
> 注：底层生息资产（如 wstETH、sUSDS）本身仍带各自协议风险，Lido/Aave 等出问题会传导至 uAsset —— 这是持有任何生息资产的固有风险，非 Outrun 机制风险。

---

## 稳健型：保本赚收益（无风险）

**画像**：持有 ETH / 稳定币 / BNB 类生息资产，想在本金绝对安全的前提下赚收益，不冒 Memecoin 价格风险。

**怎么参与（两步，都无风险）：**

1. **OutStake 质押铸 uAsset**：把生息资产铸成锚定型稳定币。
   - 想自己吃增值 → 锁仓质押，锁定期内随时提取增值。
   - 想要即时流动性 → 包装质押，拿到可流通的 uAsset。
2. **普通创世**：用 uAsset 参与某 Memecoin 创世，无风险博取额外收益。

**为什么普通创世无风险（核心机制）：**
- 你投入的 uAsset **100% 进入四池**（主池 70% + 辅助池 30%），而 Memecoin 是**全新铸造**的。
- **uAsset 在四池里守恒**：无论 Memecoin 涨跌，你投入的 uAsset 不会消失。
- Memecoin 归零时，损失的只是"Memecoin 那部分预期收益"；你通过辅助池流动性份额（含可赎回 uAsset 的 POL、锚定 uAsset 的 PT）按份额拿回投入的 uAsset 本金，一分不少。
- 创世失败则全额退款。
- 上行 Memecoin 涨，你通过 YT、辅助池份额额外赚。

**收益：**
- OutStake 端：底层生息资产增值。
- 普通创世端：**辅助池手续费分成**（只要 Memecoin 在交易就有现金流）+ **YT**（Memecoin 上涨时再加分）。
- 即 Memecoin 涨越多赚越多；就算归零，本金 uAsset 照常拿回，已累积的辅助池手续费仍是净赚。

**举例**：
> 假设用户持有 10 ETH 的 wstETH。在 OutStake 锁仓质押（锁定期天数自选，例如 90 天），铸出约 10 UETH；锁仓期间 wstETH 持续生息，可随时提取增值，本金到期赎回 —— 这一步本金锚定 ETH，与 Memecoin 无关。
>
> 用户再把部分 UETH 换成 UUSD（或直接用 UETH），拿 2000 UUSD 去参与某 Memecoin 普通创世：创世未达标全额退；达标后，这 2000 UUSD 进四池，Memecoin 涨则 YT + 辅助池份额额外增值，**Memecoin 归零则这 2000 UUSD 本金通过辅助池份额原数拿回**，期间累积的辅助池手续费仍是净赚。全程本金无虞。

---

## 平衡型：放大早期敞口

**画像**：看好某个 Memecoin，愿意承担一定成本，在创世期放大份额博取倍数收益。

**怎么参与**：
1. **杠杆创世**：支付 uAsset（或创世积分）利息，放大创世份额。
2. 叠加**预购**，以固定低费率锁定更多早期份额。

**收益**：
- 杠杆：按付息比例分得杠杆 YT；结算时若 Memecoin 大涨，从结算残值按比例分得丰厚收益。
- 预购：固定 1% 费率结算，线性解锁 Memecoin。

**下行风险**：**仅有利息成本**。杠杆创世不投入本金，只付利息换份额，所以损失上限就是付的利息；Memecoin 大跌只是让 YT/残值归零（损失=已付利息），不会亏更多。**无清算风险**（不依赖预言机、不会倒欠）；创世失败则利息全额退还。

**举例**：
> 假设用户强烈看好某 Memecoin，用杠杆创世：支付 500 UUSD 利息，按当时利率（假设对应 5 倍放大）换得约 2500 UUSD 等值的创世份额；同时预购再锁定一部分。Memecoin 锁定后，用户按付息比例分得杠杆 YT；结算时若大涨，从结算残值里分得可观收益。
>
> 代价：500 UUSD 利息在锁定时归协议自留国库；若 Memecoin 归零，损失这 500 利息，但不会被清算、不倒欠。若创世未达标，500 UUSD 全额退还。

---

## 共建型：深度参与社区

**画像**：相信某个 Memecoin 社区的长期价值，愿意直接持有 Memecoin，通过质押和治理参与共建。

**怎么参与**：
1. Memecoin 上线后，在公开市场**买入 Memecoin**。
2. 把 Memecoin **质押进收益库**，赚交易手续费收益。
3. 质押份额自带**投票权**，参与 DAO 提案、投票，共同决定国库与社区方向。

**收益**：
- **Staking 收益**：该 Memecoin 交易手续费（Memecoin 计价部分）持续流入收益库，份额升值。
- **周期激励**：按治理参与活跃度，定期获得 DAO 激励。
- **治理话语权**：决定国库资金用途、社区方向。
- **上涨收益**：直接持有 Memecoin，享受价格上涨。

**下行风险**：**直接持有 Memecoin，承担其价格波动**（这是三档中唯一本金暴露于 Memecoin 价格的）；若 Memecoin 大跌，持仓价值缩水。但通过 Staking 持续赚手续费 + 治理激励，把单纯投机转为长期共建。

**举例**：
> 某 Memecoin 上线后，用户买入并质押进收益库。该 Memecoin 交易越活跃，流入收益库的手续费越多，份额持续升值，赎回时拿回更多 Memecoin；同时份额带投票权，用户参与提案（如决定国库 UUSD 用于社区空投），按周期获激励。代价是直接持有 Memecoin、承担其价格波动 —— 但用户看中的是社区长期价值，用质押 + 治理把投机变成共建。

---

## 其他角色

除了三档参与者，生态里还有几个专门角色：

| 角色 | 做什么 | 赚什么 |
|---|---|---|
| **发起人** | 在多链启动一个 Memecoin、设定规则 | 社区共识（无法在锁定期抽走流动性） |
| **流动性提供者(LP)** | Memecoin 上线后用 uAsset + Memecoin 加池 | LP 手续费分成 |
| **执行者** | 任何人触发手续费分发 | 主池 uAsset 费 0.25% 的执行奖励（类似 keeper） |

---

## 你可以组合

三档不是互斥的。一位用户完全可以同时：
1. 在 OutStake 锁仓质押铸 UETH（**稳健**）
2. 拿 UETH 参与普通创世，无风险博取 Memecoin 收益（**稳健**，本金 uAsset 守恒）
3. 对特别看好的用杠杆 + 预购放大（**平衡**）
4. 上线后买入并质押，参与 DAO 共建（**共建**）

Outrun 的设计就是让这些动作，在同一个 uAsset 体系里顺畅串联 —— 这正是「双模块生态」相对单点产品的价值：不同需求的人，在同一套基础设施里各取所需，又共同为飞轮贡献动力。
