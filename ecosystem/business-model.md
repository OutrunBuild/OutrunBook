...
按风险偏好分三档：**稳健型**（OutStake 质押铸 uAsset + 普通创世，全程无风险 —— 本金 uAsset 守恒）→ **平衡型**（杠杆创世 + 预购，仅利息成本）→ **共建型**（买入并质押 Memecoin、参与 DAO 治理，深度共建）。另有发起人、LP、执行者等专门角色。详见[参与方式与风险偏好](playbooks.md)。
