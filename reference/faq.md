...
> 技术注：代码层 uAsset 以"债务账本"记账（每个质押管理器记录其未偿铸币量，由质押的 SY 资产覆盖），产品层表现为锚定稳定币。正常机制下 SY 的增值恒覆盖债务，故 1 uAsset 始终对应 1 单位底层资产价值；若底层汇率在极端市场条件下长期大幅回落，共享池不足额期间赎回整笔暂不可执行（不按比例折价兑付），待覆盖恢复后按面值继续（详见 [双质押模式](../outstake/staking-modes.md) 的风险说明）。
