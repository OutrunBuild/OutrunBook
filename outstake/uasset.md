...
**锚定的含义：1 枚 uAsset 始终对应 1 单位底层资产价值。** 1 UETH 对应 1 ETH 的价值，1 UUSD 对应 1 美元，1 UBNB 对应 1 BNB。这一对应按面值记账；把 uAsset 兑回底层资产时，实际兑付受共享池余额与底层汇率制约 —— 正常机制下按面值足额兑付；若极端市场条件下底层汇率长期、大幅回落导致共享池不足额，赎回整笔暂不可执行（不按比例折价兑付），待池子重新足额覆盖后按面值继续（详见 [双质押模式](staking-modes.md) 的风险说明）。
